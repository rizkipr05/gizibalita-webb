...
/* =========================
   3. Tentukan UMUR (ikut pemeriksaan_list)
   ========================= */
/**
 * Logika:
 * - Kalau SUDAH ada riwayat pemeriksaan:
 *      umur_bulan = umur_bulan dari pemeriksaan TERAKHIR
 *   -> supaya sama persis dengan tampilan di pemeriksaan_list.php
 * - Kalau BELUM ada riwayat:
 *      fallback hitung dari tanggal_lahir (kalau ada)
 */
$umur_bulan = "-";

if (!empty($riwayat)) {
    $last       = end($riwayat);           // pemeriksaan terakhir
    ...
    <div class="row g-4">
          ...
          <div class="card-body">
            ...
            <?php if ($balita): ?>
              ...
              <?php if (!empty($riwayat)): ?>
                <a href="<?= BASE_URL ?>/nakes/pemeriksaan_input.php?balita_id=<?= (int)$balita['id']; ?>"
      ...
      <div class="col-12 col-lg-7">
        ...
        <div class="card card-soft mb-4">
          <div class="card-body">
            <h6 class="text-uppercase text-muted mb-3">Grafik Perkembangan Gizi</h6>

            <?php if (!empty($riwayat)): ?>
              <p class="small text-muted mb-3">
                Grafik berikut menampilkan perkembangan <strong>berat badan (kg)</strong>,
                <strong>tinggi badan (cm)</strong>, dan <strong>lingkar lengan (cm)</strong>
            ...
            <?php else: ?>
              <p class="text-muted small mb-0">
                Belum ada riwayat pemeriksaan untuk balita ini. Grafik akan muncul setelah
                minimal satu pemeriksaan dicatat.
              </p>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div class="card card-soft">
          <div class="card-body">
            <h6 class="text-uppercase text-muted mb-3">Riwayat Pemeriksaan</h6>
                ...
                <table class="table align-middle mb-0">
                  ...
                  <tbody>
                    ...
                    <?php foreach ($riwayat as $rw): ?>
                      <tr>
                        <td><?= $no++; ?></td>
                        <td><?= htmlspecialchars($rw['tanggal']); ?></td>
                        <td><?= (int)$rw['umur_bulan']; ?></td>
                        <td><?= number_format($rw['berat_badan'], 1); ?></td>
                        <td><?= number_format($rw['tinggi_badan'], 1); ?></td>
                        <td>
                          <?= $rw['lingkar_lengan'] !== null
                                ? number_format($rw['lingkar_lengan'], 1)
                                : '-'; ?>
                        </td>
                        <td>
                          <span class="badge badge-status <?= badgeClass($rw['status_gizi']); ?>">
                            <?= htmlspecialchars($rw['status_gizi']); ?>
                          </span>
                        </td>
                      </tr>
                    <?php endforeach; ?>
            ...
            <?php else: ?>
              <p class="text-muted small mb-0">
                Belum ada data pemeriksaan untuk balita ini.
              </p>
            <?php endif; ?>
...
<?php if (!empty($riwayat)): ?>
  ...
  <script>
    ...
    const berat    = <?= $chart_berat_json ?>;
    const tinggi   = <?= $chart_tinggi_json ?>;
    const lila     = <?= $chart_lila_json ?>;
